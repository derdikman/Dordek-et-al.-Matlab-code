...
%
%% my function - Yedidyah

 spacing = size;
origin1 = [ origin,spacing];  %TODO - expand for 2d origins
origin2 = [spacing/2*sqrt(3), spacing/2];
 step2 = [ spacing*sqrt(3), spacing];  % Hexagons are compsed of 2 square grids shifted from eachother by spacing*root(3)
  step1 = [ spacing*sqrt(3), spacing];
pts1 = squareGrid(bounds, origin1, step1)';
pts2 = squareGrid(bounds, origin2, step2)';

pts = [pts1,pts2];

 varargout{1} = pts;
%  
%  figure; plot(out1(1,:),out1(2,:),'.'), hold on
